...
def _fmt_job(j, lng):
    ...
    lbl_lang = "🌐 *Language:* "         if lng == "en" else "🌐 *ቋንቋ:* "
...
async def inline_search(upd: Update, ctx):
    query = upd.inline_query.query.strip()
    if not query:
        return
    ...
    for j in results[:15]:
        ...
        inline_results.append(
            InlineQueryResultArticle(
                id=str(uuid.uuid4()), 
                title=f"📋 {j.get('title', 'N/A')}",
                description=f"🏢 {j.get('company', 'N/A')} - 📍 {j.get('location', 'N/A')}",
    ...
    await upd.inline_query.answer(inline_results, cache_time=10)
        ...
        if act == "broadcast":
            ...
            for u in users:
                ...
                if kw_target in user_kws or kw_target in user_cats:
                    try:
                        await ctx.bot.send_message(chat_id=target_uid, text=f"🔔 *MuyaSeekerAlert*\n\n{body}", parse_mode="Markdown")
                        count += 1
                        await asyncio.sleep(0.05)
                    except: pass
